...
The whole thing runs in a Y2K / Windows-98 fake desktop — draggable `.exe`
windows, a Start menu, a scrolling webring, and a Space Jam '96 starfield drawn
live with p5.js.
...
"quasi-RGB" mode stacks 2–3 `<use>` of the same symbol per cell to fake colour.
...
- **No runtime dependencies.** Canvas, SVG, and the File API are all native; a few
  lines beat a dependency. (Dev-only deps — the GIF encoder, p5 from CDN for the
  backdrop — are fine.)
...
public/
  spacejam.js   the p5.js Space Jam '96 backdrop
